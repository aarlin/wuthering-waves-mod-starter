...
class ModManager {
  static Settings = {
    ...
    Uid: " | Github:https://github.com/aarlin/wuthering-waves-mod-starter",
  };
  ...
  static listenModsToggle() {
    ...
    //   ModDebugger_1.ModDebugger.playerdebug();
    // }
    ...
    if (this.listenMod("HitMultiplier", "F6", "HitMultiplier")) {
      if (this.Settings.HitMultiplier) {
        ModUtils_1.ModUtils.KuroSingleInputBox({
          title: ModTranslation("HitMultiplier: Please enter hit count"),
          ...
          defaultText: ModTranslation("Please enter hit count"),
          isCheckNone: true,
  ...
  static ShowFuncStateTip(func, string) {
    string = ModTranslation(string);
    var info = "Unknown";
    if (this.Settings.hasOwnProperty(func)) var state = this.Settings[func];
    if (state) {
      info = string + " | " + ModTranslation("ON");
      this.ShowTip(info);
    } else {
      info = string + " | " + ModTranslation("OFF");
      this.ShowTip(info);
    }
  }
  ...
  static ShowMenu() {
    ...
    const cheatToggleStatus = [
      this.FuncState(this.Settings.GodMode, ModTranslation("GodMode[F5]")),
      this.FuncState(this.Settings.HitMultiplier, ModTranslation("HitMultiplier[F6]")),
      this.FuncState(this.Settings.AutoPickTreasure, ModTranslation("AutoPickTreasure[F7]")),
      this.FuncState(this.Settings.AutoAbsorb, ModTranslation("AutoAbsorb[F8]")),
      this.FuncState(this.Settings.killAura, ModTranslation("killAura[F9]")),
      this.FuncState(this.Settings.PerceptionRange, ModTranslation("PerceptionRange[F10]")),
      this.FuncState(this.Settings.NoCD, ModTranslation("NoCD[F11]")),
      this.FuncState(this.Settings.PlayerSpeed, ModTranslation("PlayerSpeed[F12]")),
      this.FuncState(this.Settings.CustomTp, ModTranslation("CustomTp[Ins]")),
      this.FuncState(this.Settings.AutoLoot, ModTranslation("AutoLoot[Num0]")),
      this.FuncState(this.Settings.AntiDither, ModTranslation("AntiDither[Num1]"))
    ];

    const state = cheatToggleStatus.join(' ');

    //this.FuncState(this.Settings.MarkTp,"MarkTp")+
    //this.FuncState(this.Settings.InfiniteStamina,"InfiniteStamina")

    newConfirmBox.SetTextArgs(state);
    newConfirmBox.SetTitle(ModTranslation("NotKunMods State[Home] DisableAntiCheat : ON "));
