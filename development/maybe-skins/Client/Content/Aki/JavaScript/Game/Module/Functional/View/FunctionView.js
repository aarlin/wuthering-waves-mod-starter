...
class FunctionView extends UiViewBase_1.UiViewBase {
		...
		super(...arguments),
			...
			(this.m7t = () => {
				...
				0 === e
					...
					: ((t = Math.floor(e / 100)),
						(e %= 100),
						LguiUtil_1.LguiUtil.SetLocalText(n, "BirthDay", t, e));
			}),
			(this.d7t = (t) => {
				this.BNe(t), this.B9t.SetToggleSelectByIndex(t);
			}),
			(this.C7t = (t, e, n) => (
				(t = new FunctionAttachItemGrid_1.FunctionAttachItemGrid(
					t,
				)).SetNeedAnim(0 === e),
				this.H9t.set(e, t),
				t
	...
	OnStart() {
		...
		var t =
			((t = this.GetItem(29)).SetUIActive(!1),
	...
	f7t(t) {
		var e = [];
		for (let n = 0, o = t; n < o; ++n) e.push(...this.uft[n]);
		for (const t of e) {
			var n =
				ModelManager_1.ModelManager.FunctionModel.GetFunctionItemRedDotName(t);
			if (
				n &&
				(n = ModelManager_1.ModelManager.RedDotModel.GetRedDot(n)) &&
				n.IsRedDotActive()
			)
				return !0;
		}
	...
	p7t(t) {
		var e = [];
		for (let n = t + 1, o = this.uft.length; n < o; ++n) e.push(...this.uft[n]);
		for (const t of e) {
			var n =
				ModelManager_1.ModelManager.FunctionModel.GetFunctionItemRedDotName(t);
			if (
	...
	E7t() {
		...
		LguiUtil_1.LguiUtil.SetLocalText(this.GetText(4), "PlayerLevelNum", e),
			...
			(e = PlayerExpByPlayerLevel_1.configPlayerExpByPlayerLevel.GetConfig(e));
		e?.PlayerExp &&
	...
	g7t() {
		...
		this.uft = [];
		let n = 0;
		for (; e > n + this.j9t.TotalGridNumber; )
			this.uft.push(t.slice(n, n + this.j9t.TotalGridNumber)),
				(n += this.j9t.TotalGridNumber);
		this.uft.push(t.slice(n, e));
		var o = Math.ceil(e / this.j9t.TotalGridNumber);
		this.B9t.RefreshTab(o),
			this.Wft.SetBoundDistance(0),
			this.Wft.ReloadView(o, this.uft);
	}
	L7t(t) {
		var e = this.uft;
		let n = -1;
		for (let o = 0, i = e.length; o < i; ++o) e[o].includes(t) && (n = o);
		return n;
	}
	GetGuideUiItemAndUiItemForShowEx(t) {
		if (1 < t.length || isNaN(Number(t[0])))
			...
			else if ((this.Wft.AttachToIndex(e, !0), (e = this.H9t.get(e)))) {
				if ((e = e.GetFunctionItem(t)).GetActive() && (e = e.GetButtonItem()))
					return [e, e];
			} else
				Log_1.Log.CheckError() &&
					Log_1.Log.Error(
	...
	T7t() {
		var t = this.GetItem(29),
			e = (o = this.GetGridLayout(30)).GetCellSize(),
			n = o.GetSpacing(),
			o = o.GetPadding(),
			i = t.GetWidth(),
			r = ((t = t.GetHeight()), (i - o.Left - o.Right) % (e.X + n.X)),
			a = r >= e.X ? 1 : 0;
		(i = Math.floor((i - o.Left - o.Right) / (e.X + n.X)) + a),
			(a = (t - o.Top - o.Bottom) % (e.Y + n.Y) > e.Y ? 1 : 0);
		return {
			TotalGridNumber:
				i * (Math.floor((t - o.Top - o.Bottom) / (e.Y + n.Y)) + a),
			OffsetWidth: r >= e.X ? r - e.X : r + n.X,
		};
	}
}
