...
class PreloadController extends ControllerBase_1.ControllerBase {
			...
			if (0 === a.length) t(!0);
				...
				for (const e of a) {
					...
					e.IsInit ||
						i.GetLoading() ||
						i.GetRemoveState() ||
						i.GetEntityType() === Protocol_1.Aki.Protocol.wks.Proto_Custom ||
	...
	static PreloadEntity(e, t, o) {
		if (ModelManager_1.ModelManager.PreloadModel.IsUsePreload) {
			...
			if (
				...
				r?.Start(),
				a.GetEntityType() === Protocol_1.Aki.Protocol.wks.Proto_Custom)
			)
				r?.Stop(), o?.(!0);
			else {
				s?.Start();
		...
		if (r) {
			if (
				...
				t.GetEntityType() === Protocol_1.Aki.Protocol.wks.Proto_Player ||
					t.GetEntityType() === Protocol_1.Aki.Protocol.wks.Proto_Monster ||
					t.GetEntityType() === Protocol_1.Aki.Protocol.wks.Proto_Vision)
			) {
	...
	static Cfr(e) {
		...
		} else if (t.GetEntityType() === Protocol_1.Aki.Protocol.wks.Proto_Vision) {
			(C = (0, puerts_1.$ref)(void 0)),
				UE.DataTableFunctionLibrary.GetDataTableRowNames(
					o.GetVisionCommonSkillInfo(),
					C,
